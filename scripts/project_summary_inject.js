async function injectProjectSummaries() {
    ...
    try {
        ...
        const sectionDescriptions = projectsData.sectionDescriptions || {};
        ...
        function formatProjectDuration(durationValue) {
            if (durationValue === null || durationValue === undefined) return '';

            const raw = String(durationValue).trim();
            if (!raw) return '';

            const shortWeekMatch = raw.match(/^(\d+)\s*weeks?$/i);
            if (shortWeekMatch) {
                return `${shortWeekMatch[1]}w`;
            }

            const rangeMatch = raw.match(/^(\d{1,2}\/\d{1,2}-\d{4})\s*-\s*(\d{1,2}\/\d{1,2}-\d{4})?\s*$/);
            if (!rangeMatch) return raw;

            const startDate = parseProjectDate(rangeMatch[1]);
            if (!startDate) return raw;

            let endDate;
            if (rangeMatch[2]) {
                endDate = parseProjectDate(rangeMatch[2]);
            ...
            if (endDate.getTime() < startDate.getTime()) return raw;

            const millisecondsPerWeek = 1000 * 60 * 60 * 24 * 7;
            const totalWeeks = Math.floor((endDate.getTime() - startDate.getTime()) / millisecondsPerWeek);
            return `${totalWeeks}w`;
        }
        ...
        function formatProjectMeta(project) {
            ...
            const hasTeamSize = project.teamSize !== undefined && project.teamSize !== null && String(project.teamSize).trim() !== '';
            if (hasTeamSize) {
                const rawTeamSize = String(project.teamSize).trim();
                const numericTeamSize = Number(rawTeamSize);

                if (Number.isInteger(numericTeamSize) && numericTeamSize > 0) {
                    if (numericTeamSize === 1) {
                        parts.push('Solo');
                    } else {
                        parts.push(`Team ${numericTeamSize}`);
                    }
                } else {
                    parts.push(rawTeamSize);
                }
            }
        ...
        function createProjectCard(project) {
            ...
            const metaLabel = formatProjectMeta(project);
            ...
            ${renderToolbar(project.links || {})}
                        ...
                        <p class="project-card__title"><strong>${project.title}</strong>${metaLabel ? `<span class="project-card__duration">${metaLabel}</span>` : ''}</p>
    ...
    `;
        ...
        const hasSections = projects.some(project => {
            return typeof project.section === 'string' && project.section.trim() !== '';
        });
        ...
        if (hasSections) {
            ...
            projects.forEach(project => {
                const sectionName = typeof project.section === 'string' && project.section.trim()
            ...
            sectionMap.forEach((sectionProjects, sectionName) => {
                groupedProjects.push({ sectionName, projects: sectionProjects });
            });
        } else {
            groupedProjects.push({ sectionName: '', projects });
        }

        groupedProjects.forEach(group => {
            const highlightedIndex = group.projects.findIndex(p => p.highlighted === true);
            if (highlightedIndex > 0) {
                const [highlighted] = group.projects.splice(highlightedIndex, 1);
                group.projects.unshift(highlighted);
            }
        });

        groupedProjects.forEach(group => {
            const sectionContainer = hasSections
                ? document.createElement('div')
                : contentContainer;

            if (hasSections) {
                sectionContainer.classList.add('project-card-section');
                const heading = document.createElement('h2');
                heading.classList.add('h3', 'project-card-section__title');
                heading.textContent = group.sectionName;
                sectionContainer.appendChild(heading);

                const rawDescription = sectionDescriptions[group.sectionName];
                const sectionDescription = typeof rawDescription === 'string' ? rawDescription.trim() : '';
                if (sectionDescription) {
                    const description = document.createElement('p');
                    description.classList.add('project-card-section__description');
            ...
            grid.classList.add('project-card-grid');
